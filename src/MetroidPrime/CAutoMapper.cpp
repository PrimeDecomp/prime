...
static inline float min_val(float a, float b) { return a < b ? a : b; }
static inline float max_val(float a, float b) { return a > b ? a : b; }
...
void CAutoMapper::Update(float dt, const CStateManager& mgr) {
  ...
  if (x28_frmeMapScreen.get() != NULL && x2c_frmeInitialized == NULL) {
    if (x28_frmeMapScreen->TryCache()) {
      ...
      CGuiTextPane* leftPane = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_left"));
      leftPane->TextSupport().SetText(rstl::wstring(gpStringTable->GetString(0x2a)));

      CGuiTextPane* yicon = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_yicon"));
      yicon->TextSupport().SetText(rstl::wstring(gpStringTable->GetString(0x2b)));

      x2fc_textpane_hint = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_hint"));
      x300_textpane_instructions = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_instructions"));
      x304_textpane_instructions1 = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_instructions1"));
      x308_textpane_instructions2 = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_instructions2"));

      CGuiTextPane* mapLegend = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_mapLegend"));
      ...
      x2f8_textpane_areaname = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_areaname"));
  ...
  if (x2c_frmeInitialized != NULL) {
    ...
    CGuiTextPane* right1 = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_right1"));
    ...
    if (x1bc_state == kAMS_MapScreenUniverse ||
      ...
      rstl::string hexStr(CBasics::Stringize("%8.8X", gpTweakPlayerRes->x98_aButton[x2f4_aButtonPos]));
    ...
    CGuiTextPane* right = static_cast< CGuiTextPane* >(x2c_frmeInitialized->FindWidget("textpane_right"));
  ...
  if (IsRenderStateInterpolating()) {
    ...
    SAutoMapperRenderState::InterpolateWithClamp(x160_renderState2, xa8_renderState0,
                                                  x104_renderState1,
...
void CAutoMapper::Draw(const CStateManager& mgr, const CTransform4f& xf, float alpha) const {
  ...
  float yScale = xa8_renderState0.x18_camDist /
                 static_cast< float >(tan(
                     M_PIF / 2.f - 0.5f * camAngleRad));

  CTransform4f camXf =
      xa8_renderState0.x8_camOrientation.BuildTransform4f(xa8_renderState0.x20_areaPoint);

  CTransform4f distScale(1.f / (yScale * aspect), 0.f, 0.f, 0.f,
                         0.f, 0.001f, 0.f, 0.f,
                         0.f, 0.f, 1.f / yScale, 0.f);

  CTransform4f tweakScale =
      CTransform4f::Scale(gpTweakAutoMapper->xc4_mapPlaneScaleX, 0.f,
  ...
  if (IsFullyOutOfMiniMapState()) {
    if (universeInterp < 1.f && x24_world != NULL) {
      ...
      if (!x1e0_hintSteps.empty() &&
        ...
        if (xa0_curAreaId == mgr.GetNextAreaId() && x24_world == mgr.GetWorld()) {
          float pulseTime =
              static_cast< float >(fmod(hintStepFloat * 8.f, 1.0));
          hintFlash = 2.f * (pulseTime < 0.5f ? pulseTime : 1.f - pulseTime);
        } else {
          rstl::list< SAutoMapperHintLocation >::const_iterator locIt =
              x1f8_hintLocations.begin();
          for (; locIt != x1f8_hintLocations.end(); ++locIt) {
            ...
            float pulseTime = static_cast< float >(
                fmod((1.f - max_val(0.f, (hintStepFloat - 0.5f) / 0.5f)) *
                         4.f,
                     1.0));
            hintFlash = 2.f * (pulseTime < 0.5f ? pulseTime : 1.f - pulseTime);
            break;
      ...
      const CMapWorld::CMapWorldDrawParms parms(
          ...
          xa8_renderState0.x40_alphaOutlineUnvisited * alphaInterp, mapAlpha,
          mgr, modelXf, camXf, *x24_world, mwInfo,
          2.f, true, x1dc_playerFlashPulse, hintFlash, objectScale);
      mw->Draw(parms, curArea, curArea,
               xa8_renderState0.x2c_drawDepth1, xa8_renderState0.x30_drawDepth2, true);
    }
  } else if (IsInMapperState(kAMS_MiniMap)) {
    const CMapWorld* mw = x24_world->IGetMapWorld();
    CWorldState& worldState = gpGameState->StateForWorld(x24_world->IGetWorldAssetId());
    ...
    const CMapWorld::CMapWorldDrawParms parms(
        ...
        xa8_renderState0.x3c_alphaSurfaceUnvisited * alphaInterp,
        xa8_renderState0.x40_alphaOutlineUnvisited * alphaInterp, mapAlpha,
        mgr, planeXf, camXf, *x24_world, mwInfo,
        1.f, false, 0.f, 0.f, objectScale);
    mw->Draw(parms, xa0_curAreaId.value, xa4_otherAreaId.value,
             xa8_renderState0.x2c_drawDepth1, xa8_renderState0.x30_drawDepth2, false);
  } else {
    int curArea = xa0_curAreaId.value;
    const CMapWorld* mw = x24_world->IGetMapWorld();
    CWorldState& worldState = gpGameState->StateForWorld(x24_world->IGetWorldAssetId());
  ...
  if (universeInterp > 0.f) {
    ...
    const CMapUniverse::CMapUniverseDrawParms uParms(
        universeInterp, x9c_worldIdx, gpGameState->CurrentWorldAssetId(), hexIdx,
  ...
  if (!IsInMapperState(kAMS_MapScreenUniverse)) {
    ...
    if (x24_world == mgr.GetWorld()) {
      float func = CMath::Clamp(
          ...
          1.f);
      float scale = min_val(0.6f * gpTweakAutoMapper->x10_maxCamDist /
                                   gpTweakAutoMapper->xc_minCamDist,
                               objectScale);

      CEulerAngles eulers = CEulerAngles::FromTransform(
      ...
      CModelFlags flags(CModelFlags::kT_Blend, static_cast< uchar >(0),
                        static_cast< CModelFlags::EFlags >(CModelFlags::kF_DepthCompare | CModelFlags::kF_DepthGreater),
                        modColor);
      ...
      for (; locIt != x1f8_hintLocations.end(); ++locIt) {
        ...
        CGraphics::SetModelMatrix(
            ...
            CTransform4f::Translate(const_cast< CMapArea* >(mapa)
                                        ...
                                        .GetTranslation()) *
            CTransform4f::Translate(mapa->GetAreaCenterPoint()) *
            CTransform4f::Scale(objectScale) * camRot);
        ...
        if (beaconAlpha > 0.f) {
          ...
          CColor beaconColor(0xff, 0xff, 0xff,
...
void CAutoMapper::SetupHintNavigation() {
  ...
  if (curHint != NULL && const_cast< SHintState* >(curHint)->CanContinue()) {
    ...
    for (int i = 0; i < static_cast< int >(locs.size()); ++i) {
      ...
      if (loc.x0_mlvlId != curMlvl) {
        x1e0_hintSteps.push_back(SAutoMapperHintStep(SAutoMapperHintStep::kHST_SwitchToUniverse, 0));
        x1e0_hintSteps.push_back(SAutoMapperHintStep(SAutoMapperHintStep::kHST_PanToWorld, static_cast< int >(loc.x0_mlvlId)));
      ...
      x1e0_hintSteps.push_back(SAutoMapperHintStep(SAutoMapperHintStep::kHST_PanToArea, loc.x8_areaId.value));
  ...
  for (int i = 0; i < static_cast< int >(hintStates.size()); ++i) {
    ...
    for (int j = 0; j < static_cast< int >(locs.size()); ++j) {
      x1f8_hintLocations.push_back(SAutoMapperHintLocation(1, 1.f, locs[j].x0_mlvlId, locs[j].x8_areaId));
    }
  }
}
...
bool CAutoMapper::CheckDummyWorldLoad(const CStateManager& mgr) {
  ...
  if (dummyWorld != NULL) {
    if (dummyWorld->ICheckWorldComplete()) {
      ...
      CVector3f localPoint(mapuWld.GetWorldTransform().GetQuickInverse() * xa8_renderState0.x20_areaPoint);
...
void CAutoMapper::ProcessControllerInput(const CFinalInput& input, const CStateManager& mgr) {
  if (!IsRenderStateInterpolating()) {
    bool inPlayerControl = IsInMapperState(kAMS_MapScreen) || IsInMapperState(kAMS_MapScreenUniverse);
    if (inPlayerControl) {
  ...
  if (IsInMapperState(kAMS_MapScreen)) {
    ...
    CVector3f camDir = camRot.GetColumn(kDY);
    CUnitVector3f unitDir(camDir.GetX(), camDir.GetY(), camDir.GetZ());
    int aid = FindClosestVisibleArea(xa8_renderState0.x20_areaPoint,
                                     unitDir, mgr, *x24_world, *mwInfo);
    ...
    if (static_cast< int >(x1e0_hintSteps.size()) > 0) {
      ...
      if (nextStep.x0_type == SAutoMapperHintStep::kHST_PanToWorld ||
        ...
        CVector3f camDir = camRot.GetColumn(kDY);
        CUnitVector3f unitDir(camDir.GetX(), camDir.GetY(), camDir.GetZ());
        SClosestWorldResult result = FindClosestVisibleWorld(
            xa8_renderState0.x20_areaPoint, unitDir, mgr);
...
void CAutoMapper::ProcessMapScreenInput(const CFinalInput& input, const CStateManager& mgr) {
  ...
  if (x1bc_state == kAMS_MapScreen) {
    ...
    CVector3f pointLocal = mapuWld.GetWorldTransform().GetQuickInverse() * xa8_renderState0.x20_areaPoint;
    if (mapuWld.GetWorldAssetId() != gpGameState->CurrentWorldAssetId()) {
      ...
      xa0_curAreaId.value = FindClosestVisibleArea(
          pointLocal, CUnitVector3f(camDir.GetX(), camDir.GetY(), camDir.GetZ()),
          mgr, *x24_world, *mwInfo);
      BeginMapperStateTransition(kAMS_MapScreen, mgr);
    }
  }
...
void CAutoMapper::ProcessMapRotateInput(const CFinalInput& input, const CStateManager& mgr) {
  ...
  if (dirs0 > 0.f || dirs1 > 0.f || dirs2 > 0.f || dirs3 > 0.f) {
    ...
    CQuaternion q = CQuaternion::ZRotation(angZ) *
                    CQuaternion::XRotation(angX) *
...
void CAutoMapper::ProcessMapZoomInput(const CFinalInput& input, const CStateManager& mgr) {
  ...
  if (x324_zoomState == kZS_In) {
    xa8_renderState0.x18_camDist = GetClampedMapScreenCameraDistance(xa8_renderState0.x18_camDist - delta);
    x2f0_rTriggerPos = 1;
...
void CAutoMapper::ProcessMapPanInput(const CFinalInput& input, const CStateManager& mgr) {
  ...
  if (forward > 0.f || back > 0.f || left > 0.f || right > 0.f) {
    ...
    if (x1bc_state == kAMS_MapScreen) {
      ...
      CVector3f worldPoint = const_cast< CMapArea* >(area)->GetAreaPostTransform(
          *x24_world, xa0_curAreaId.value) * area->GetAreaCenterPoint();
      ...
      CVector3f camDir = camRot.GetColumn(kDY);
      SClosestWorldResult result = FindClosestVisibleWorld(
          xa8_renderState0.x20_areaPoint, CUnitVector3f(camDir.GetX(), camDir.GetY(), camDir.GetZ()), mgr);
      const CTransform4f& hex = x8_mapu.GetObject()->GetMapWorldData(result.x0_worldIdx).GetMapAreaData(result.x4_areaIdx);
...
CAutoMapper::SAutoMapperRenderState CAutoMapper::BuildMiniMapWorldRenderState(
    const CStateManager& stateMgr, const CQuaternion& rot, int area) const {
...
CAutoMapper::SAutoMapperRenderState CAutoMapper::BuildMapScreenWorldRenderState(
    const CStateManager& mgr, const CQuaternion& rot, int area, bool doingHint) const {
...
CAutoMapper::SAutoMapperRenderState CAutoMapper::BuildMapScreenUniverseRenderState(
    const CStateManager& mgr, const CQuaternion& rot, int area) const {
  SAutoMapperRenderState ret(
...
void CAutoMapper::SetShouldPanningSoundBePlaying(bool shouldBePlaying) {
  if (shouldBePlaying) {
    if (!x1cc_panningSfx)
      x1cc_panningSfx = CSfxManager::SfxStart(0x57E, 127, 64, false,
...
void CAutoMapper::SetShouldZoomingSoundBePlaying(bool shouldBePlaying) {
  if (shouldBePlaying) {
    if (!x1d4_zoomingSfx)
      x1d4_zoomingSfx = CSfxManager::SfxStart(0x560, 127, 64, false,
                                               CSfxManager::kMedPriority, true,
                                               CSfxManager::kAllAreas);
  } else {
...
void CAutoMapper::SetShouldRotatingSoundBePlaying(bool shouldBePlaying) {
  if (shouldBePlaying) {
    if (!x1d0_rotatingSfx)
      x1d0_rotatingSfx = CSfxManager::SfxStart(0x55F, 127, 64, false,
                                                CSfxManager::kMedPriority, true,
                                                CSfxManager::kAllAreas);
  } else {
...
void CAutoMapper::BeginMapperStateTransition(EAutoMapperState state, const CStateManager& mgr) {
  ...
  if (x1bc_state == kAMS_MiniMap && state == kAMS_MapScreen) {
    x104_renderState1 =
        BuildMapScreenWorldRenderState(mgr, xa8_renderState0.x8_camOrientation, xa0_curAreaId.value, false);
    ...
    CSfxManager::SfxStart(0x592, 127, 64, false, CSfxManager::kMedPriority, false,
                          CSfxManager::kAllAreas);
    x104_renderState1 =
        BuildMapScreenUniverseRenderState(mgr, xa8_renderState0.x8_camOrientation, xa0_curAreaId.value);
    ...
    CSfxManager::SfxStart(0x593, 127, 64, false, CSfxManager::kMedPriority, false,
                          CSfxManager::kAllAreas);
    x104_renderState1 =
        BuildMapScreenWorldRenderState(mgr, xa8_renderState0.x8_camOrientation, xa0_curAreaId.value,
                                       x1e0_hintSteps.size());
...
CAutoMapper::SClosestWorldResult CAutoMapper::FindClosestVisibleWorld(
    const CVector3f& point, const CUnitVector3f& camDir, const CStateManager& mgr) const {
...
float CAutoMapper::GetDesiredMiniMapCameraDistance(const CStateManager& mgr) const {
  ...
  for (int i = -1; i < (oneMiniMapArea ? 0 : static_cast< int >(area->IGetNumAttachedAreas())); ++i) {
  ...
  return (0.5f * (0.5f * extent.Magnitude()) + 0.5f * maxMargin.Magnitude()) *
         ...
         static_cast< float >(tan(M_PIF / 2.f - 0.5f * 2.f * M_PIF *
                                   (xa8_renderState0.x1c_camAngle / 360.f)));
}
...
float CAutoMapper::GetClampedMapScreenCameraDistance(float value) const {
  ...
  float clamped = CMath::Clamp(gpTweakAutoMapper->xc_minCamDist, value,
                               gpTweakAutoMapper->x10_maxCamDist);
  return clamped;
}
...
bool CAutoMapper::IsRenderStateInterpolating() const {
  return x1c8_interpTime < x1c4_interpDur;
}

bool CAutoMapper::IsInMapperStateTransition() const {
  return x1bc_state != x1c0_nextState;
}
...
void CAutoMapper::TransformRenderStatesWorldToUniverse() {
  const CTransform4f& xf =
      x8_mapu.GetObject()->GetMapWorldData(x9c_worldIdx).GetWorldTransform();
...
CAutoMapper::SAutoMapperRenderState::SAutoMapperRenderState(
    const CVector2i& viewportSize, const CQuaternion& camOrientation, float camDist,
    float camAngle, const CVector3f& areaPoint, float drawDepth1, float drawDepth2,
...
void CAutoMapper::SAutoMapperRenderState::InterpolateWithClamp(
    const SAutoMapperRenderState& a, SAutoMapperRenderState& out,
  ...
  if (b.x44_viewportEase != kE_None) {
    ...
    out.x0_viewportSize = CVector2i(
        static_cast< int >(easeA * a.x0_viewportSize.GetX() + easeB * b.x0_viewportSize.GetX()),
        static_cast< int >(easeA * a.x0_viewportSize.GetY() + easeB * b.x0_viewportSize.GetY()));
  }
  ...
  if (b.x48_camEase != kE_None) {
    ...
    float easeA = 1.f - easeB;
    out.x18_camDist = a.x18_camDist * easeA + b.x18_camDist * easeB;
    out.x1c_camAngle = a.x1c_camAngle * easeA + b.x1c_camAngle * easeB;
  }

  if (b.x4c_pointEase != kE_None) {
    float eB = eases[b.x4c_pointEase];
    float eA = 1.f - eB;
    out.x20_areaPoint = CVector3f(
        a.x20_areaPoint.GetX() * eA + b.x20_areaPoint.GetX() * eB,
        a.x20_areaPoint.GetY() * eA + b.x20_areaPoint.GetY() * eB,
        a.x20_areaPoint.GetZ() * eA + b.x20_areaPoint.GetZ() * eB);
  }

  if (b.x50_depth1Ease != kE_None) {
    float eB = eases[b.x50_depth1Ease];
    out.x2c_drawDepth1 = a.x2c_drawDepth1 * (1.f - eB) + b.x2c_drawDepth1 * eB;
  }

  if (b.x54_depth2Ease != kE_None) {
    float eB = eases[b.x54_depth2Ease];
    out.x30_drawDepth2 = a.x30_drawDepth2 * (1.f - eB) + b.x30_drawDepth2 * eB;
  }

  if (b.x58_alphaEase != kE_None) {
    float eB = eases[b.x58_alphaEase];
    float eA = 1.f - eB;
    out.x34_alphaSurfaceVisited = a.x34_alphaSurfaceVisited * eA + b.x34_alphaSurfaceVisited * eB;
    out.x38_alphaOutlineVisited = a.x38_alphaOutlineVisited * eA + b.x38_alphaOutlineVisited * eB;
    out.x3c_alphaSurfaceUnvisited = a.x3c_alphaSurfaceUnvisited * eA + b.x3c_alphaSurfaceUnvisited * eB;
    out.x40_alphaOutlineUnvisited = a.x40_alphaOutlineUnvisited * eA + b.x40_alphaOutlineUnvisited * eB;
  }
}
...
CAutoMapper::SAutoMapperHintLocation::SAutoMapperHintLocation(
    uint showBeacon, float beaconAlpha, CAssetId worldId, TAreaId areaId)
: x0_showBeacon(showBeacon)
, x4_beaconAlpha(beaconAlpha)
, x8_worldId(worldId)
, xc_areaId(areaId) {}

CAutoMapper::SAutoMapperRenderState::SAutoMapperRenderState(
    const SAutoMapperRenderState& other)
...
, xa0_curAreaId(x24_world->IGetCurrentAreaId())
, xa4_otherAreaId(xa0_curAreaId)
, xa8_renderState0(BuildMiniMapWorldRenderState(stateMgr,
    CQuaternion::FromMatrix(stateMgr.GetCameraManager()->GetCurrentCamera(stateMgr).GetTransform()),
    xa0_curAreaId.value))
, x104_renderState1(xa8_renderState0)
, x160_renderState2(xa8_renderState0)
...
, x32c_loadingDummyWorld(false) {
  ...
  x48_mapIcons.push_back(gpSimplePool->GetObj(SObjectTag('TXTR', gpTweakPlayerRes->x4_saveStationIcon)));
  x48_mapIcons.push_back(gpSimplePool->GetObj(SObjectTag('TXTR', gpTweakPlayerRes->x8_missileStationIcon)));
  x48_mapIcons.push_back(gpSimplePool->GetObj(SObjectTag('TXTR', gpTweakPlayerRes->xc_elevatorIcon)));
  x48_mapIcons.push_back(gpSimplePool->GetObj(SObjectTag('TXTR', gpTweakPlayerRes->x10_minesBreakFirstTopIcon)));
