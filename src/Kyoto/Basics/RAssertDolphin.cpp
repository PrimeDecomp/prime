    ...
    if (code != 15 || ((arg1 & 0x10) == 0 && (arg2 < 0x1800001))) {
        ...
        do {
            OSReport(
                ...
                i,
                context->gpr[i], context->gpr[i], i + 0x10,
                context->gpr[i + 0x10], context->gpr[i + 0x10]
            );
            i += 1;
        } while (i < 0x10);
        ...
        bufferIndex += sprintf(rs_debugger_buffer + bufferIndex, "IP: 0x%8.8x  Mem: 0x%8.8x", context->srr0, arg2);
        if (code == 0xf) {
        ...
        while( true ) {
            ...
            bufferIndex += sprintf(rs_debugger_buffer + j,"0x%08x: 0x%08x 0x%08x\n", stackPtr, *asPtr, asPtr[1]);
            stackPtr = *asPtr;
        }
